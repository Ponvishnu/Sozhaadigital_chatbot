...
def fetch_snippets(urls, chars=2000):
    snippets = []
    headers = {"User-Agent": "SozhaaBot/1.0 (+https://sozhaa.digital)"}
    for u in urls:
        try:
...
def call_gemini(system_prompt, history, user_message):
    ...
    if "service" in user_message.lower() or "services" in user_message.lower():
        return (
            "Sozhaa Digital provides a wide range of professional services 🚀.\n"
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa Digital 🚀\nOur team will contact you soon."
