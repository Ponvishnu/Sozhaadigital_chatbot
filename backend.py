...
COMPANY_URLS = [
    "https://sozzaadigital.com/",
    "https://sozzaadigital.com/aboutus",
    "https://sozzaadigital.com/ourservices",
    "https://sozzaadigital.com/news&events",
    "https://sozzaadigital.com/career",
    "https://sozzaadigital.com/contact"
]

STORAGE_DIR = "chat_data"
TRANSCRIPT_EXCEL = os.path.join(STORAGE_DIR, "sozhaa_digital_chat_history.xlsx")
TRANSCRIPT_JSON = os.path.join(STORAGE_DIR, "sozhaa_digital_transcripts.json")
...
def fetch_snippets(urls, chars=1500):
    snippets = []
    headers = {"User-Agent": "SozhaaDigitalBot/1.0 (+https://sozzaadigital.com)"}
    for u in urls:
        try:
...
def build_system_prompt(snippets):
    ...
    return (
        ...
        "(sozzaadigital.com). Answer only about the company, Our services, news & events, careers, and contact information. "
...
def build_html_email(user_details, service, transcript):
    header = f"""
    <h2>Sozhaa Digital — Chat Transcript</h2>
...
@app.get("/")
def root():
    return {"status": "ok", "msg": "Sozhaa Digital Chatbot backend running"}
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa Digital 🚀<br>Our team will contact you soon."
        ...
        if user_phone:
            thank_msg = "✅ Thanks for contacting Sozhaa Digital. Our team will contact you soon 🚀"
        ...
        if payload.user_details.get("email"):
            thank_you_html = html + "<br><br><p>🙏 Thank you for chatting with Sozhaa Digital. Our team will connect with you soon.</p>"
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Digital — Chat Summary", thank_you_html, TRANSCRIPT_EXCEL)
    ...
    if "support" in user_msg.lower() or "contact" in user_msg.lower():
        ...
        def support_alert():
            try:
                ...
                """
                send_email_with_attachment(COMPANY_EMAIL, "⚠ Sozhaa Foundation — Support Request", alert_html)
                if payload.user_details.get("email"):
                    send_email_with_attachment(payload.user_details["email"], "Sozhaa Digital — Support Request Received", "<p>We received your request. Our team will contact you soon 🚀</p>")
            except Exception as e:
